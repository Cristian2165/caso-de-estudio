...
export const PatientsView: React.FC = () => {
  ...
  const handleRegisterPatient = () => {
    ...
    // Add to mockPatients
    const updatedPatients = [...mockPatients, newMockPatient];
  ...
  const [mockPatients, setMockPatients] = useState<MockPatient[]>(() => {
    const saved = localStorage.getItem('mockPatients');
    return saved ? JSON.parse(saved).map((p: any) => ({
      ...p,
      diagnosis: Array.isArray(p.diagnosis) ? p.diagnosis : [p.diagnosis || 'TEA Leve'],
      emotions: p.emotions || [],
      heartRate: p.heartRate || 80,
  ...
  const savePatients = (patients: MockPatient[]) => {
              ...
              <CardHeader>
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <CardDescription>{patient.age} años • {patient.diagnosis.join(', ')}</CardDescription>
                  </div>
                  <Badge 
                    variant={
